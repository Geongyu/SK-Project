...
class Encoder(nn.Module):
    ...
    *https://github.com/wolny/pytorch-3dunet/blob/master/pytorch3dunet/unet3d/model.py*
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, apply_pooling=True,
                 pool_kernel_size=(2, 2, 2), pool_stride=(2, 2, 2), pool_type='max', basic_module=DoubleConv, conv_layer_order='crg',
                 num_groups=8):
        super(Encoder, self).__init__()
        assert pool_type in ['max', 'avg']
        if apply_pooling:
            if pool_type == 'max':
                self.pooling = nn.MaxPool3d(kernel_size=pool_kernel_size, stride=pool_stride)
            else:
                self.pooling = nn.AvgPool3d(kernel_size=pool_kernel_size, stride=pool_stride)
        else:
            self.pooling = None
