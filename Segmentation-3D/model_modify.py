...
from unet3d.buildingblocks import Encoder, Decoder, DoubleConv, SingleConv
...
class UNet3D(nn.Module):
    ...
    *https://github.com/wolny/pytorch-3dunet/blob/master/pytorch3dunet/unet3d/model.py*
    ...
    def __init__(self, in_channels, out_channels, f_maps=64, depth_stride=2, conv_layer_order='crg', num_groups=8,
                 **kwargs):
        ...
        if isinstance(depth_stride, int):
            depth_stride = [0] + [depth_stride] * (len(f_maps) - 1)
        ...
        for i, out_feature_num in enumerate(f_maps):
            if i == 0:
                encoder = Encoder(in_channels, out_feature_num, apply_pooling=False,
                                  basic_module=DoubleConv, conv_layer_order=conv_layer_order, num_groups=num_groups)
            else:
                encoder = Encoder(f_maps[i - 1], out_feature_num, pool_stride=(depth_stride[i], 2, 2), basic_module=DoubleConv,
